        ...
        {body ? (
          <Accordion type="single">
            <AccordionItem value="an-item">
              ...
              >
                ...
                <MarkdownBlock
                  content={title /*+ (!nextTimestamp ? dots : '')*/}
                />
              </AccordionTrigger>
              <AccordionContent>
          ...
          >
            <div className="flex items-center justify-between gap-2 m-w-40">
              ...
              {activityType !== "info" && (
                <div className="whitespace-nowrap">
                  {getTimeDifference(timestamp, nextTimestamp || currentTime)}
                </div>
              )}
              <div className={`mx-1 w-1 h-4 border-l-2`} />
            </div>
  ...
  return (
    <Accordion
      ...
      className={`w-full border-t border-border ${
        alternateBackground === "primary" ? "bg-primary/10" : ""
      }`}
    >
      <AccordionItem value="item-1" className="border-b-0">
        ...
        <AccordionContent className="pl-4 border-b-0">
          {children?.map((child, index) => {
            ...
            const messageBody = child.message.substring(
              child.message.indexOf(" ")
            );
            return (
              <Activity
                ...
                activityType={
                  ...
                  !messageType.split("[")[3]
                    ...
                    : (messageType
                        .split("[")
                        [messageType.startsWith("[SUBACTIVITY]") ? 3 : 2].split(
                          "]"
                        )[0]
                        .toLowerCase() as
                        | "error"
